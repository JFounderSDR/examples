/***************************************************************************//**
* @file     ImageTrans_Ctroller_servant.cpp
* @author   open Team
* @version  1
* @date     2019-05-05
* @brief
...
#include "../include/ImageTrans_Ctroller_servant.h"
...
#include <usrLib.h>
#endif
ImageTrans_Ctroller_servant::ImageTrans_Ctroller_servant()
{
}

/**
 * @brief	ImageTrans_Ctroller_servant Constructor.
 * 
...
Resource_impl(_id)
{
	DEBUG(6, [ImageTrans_Ctroller_servant], "In constructor.")
...
ImageTrans_Ctroller_servant::~ImageTrans_Ctroller_servant()
{
	DEBUG(6, [ImageTrans_Ctroller_servant], "in destructor")
/**************************OPENSCA-USERREGION-BEGIN*******************************/
 ...
 */
void 
ImageTrans_Ctroller_servant::setMhalPortLD()
{
	FileSystem_impl * fs_i = new FileSystem_impl(m_fsroot.c_str());
	if(!fs_i->exists(m_spdRelPath.c_str())) {
		DEBUG(0, [ImageTrans_Ctroller_servant::setMhalPortLD],
 ...
 */
CORBA::Object_ptr 
ImageTrans_Ctroller_servant::getPort(
	const char * portName) 
throw (
	CORBA::SystemException, 
	CF::PortSupplier::UnknownPort)
{
    DEBUG(6, [ImageTrans_Ctroller_servant::getPort], "In getPort.")
		
	CORBA::Object_var _port;
	
	std::string portFullName = "OpenSCA_Domain/" + m_appName + "/" + portName;

	if ((0 == strcmp(portName, RXTXCOMP_UPORT)) ||
		(0 == strcmp(portName, RXTXCOMP_PPORT))) {
 ...
 */
void 
ImageTrans_Ctroller_servant::start()
throw (
	CORBA::SystemException, 
	CF::Resource::StartError)
{
	DEBUG(6, [ImageTrans_Ctroller_servant::start], "In start.")
/**************************OPENSCA-USERREGION-BEGIN*******************************/
	...
	if(!m_isStarted){
		...
		std::vector <CF::Resource_ptr> comps;
		comps = control_out_uport->getProvidesPorts();
		DEBUG(7, [Asscon_servant::start], " comps.size(): " << comps.size());

		for(int i = 0; i < comps.size(); i++) {
			if(CORBA::is_nil(comps[i])) {
				DEBUG(7, [Asscon_servant::start], " get component failed. ");
				break;
			}
			comps[i]->start();
		}
 ...
 */
void 
ImageTrans_Ctroller_servant::stop() 
throw (
	CORBA::SystemException, 
	CF::Resource::StopError) 
{  
	DEBUG(6, [ImageTrans_Ctroller_servant::stop], "In stop.")
/**************************OPENSCA-USERREGION-BEGIN*******************************/
#ifdef __SDS_OS_LINUX__
	if(m_isStarted){
		try{
			...
			DEBUG(0, [Asscon_servant::stop], " stop error.")
		}

		std::vector <CF::Resource_ptr> comps;
		comps = control_out_uport->getProvidesPorts();
		DEBUG(7, Asscon_servant::stop, " comps.size(): " << comps.size());

		for(int i = 0; i < comps.size(); i++) {
			if(CORBA::is_nil(comps[i])) {
				DEBUG(7, [Asscon_servant::stop], " get component failed. ");
				break;
			}

			DEBUG(7, [Asscon_servant::stop], " call stop. ");
			comps[i]->stop();
		}
		m_isStarted = false;
	}
#endif
	DEBUG(7, [Asscon_servant::stop], " leave.")
/**************************OPENSCA-USERREGION-END*********************************/
}
 ...
 */
void 
ImageTrans_Ctroller_servant::releaseObject() 
throw (
	CORBA::SystemException, 
	CF::LifeCycle::ReleaseError)
{
	DEBUG(6, [ImageTrans_Ctroller_servant::releaseObject], " In releaseObject.")
	...
	try {
		...
		DEBUG(0, [ImageTrans_Ctroller_servant::releaseObject],
 ...
 */
void 
ImageTrans_Ctroller_servant::initialize()
throw (
	CF::LifeCycle::InitializeError, 
	CORBA::SystemException)
{
	DEBUG(6, [ImageTrans_Ctroller_servant::initialize], " In initialize.")
	
    control_out_uport = new StandardInterfaces_i::Resource_u(("OpenSCA_Domain/" + m_appName + "/ImageTrans_Ctroller/control_out").c_str());
	m_initConfig = false;
...
/**************************OPENSCA-USERREGION-BEGIN*******************************/
	...
	CORBA::Object_ptr obj = 
		m_orbWrap->get_object_from_string(contextName.c_str());
	if (CORBA::is_nil(obj)) {
		DEBUG(1, [Asscon_servant::initialize], " obtain obj fail.")
		return;
	}
	...
	try {
		...
		DEBUG(1, [Asscon_servant::initialize],
	...
	try {
		...
		DEBUG(0, [Asscon_servant::initialize], " get MHAL device failed.")
	}

	if (CORBA::is_nil(m_mhalDev)) {
		DEBUG(0, [Asscon_servant::initialize], " get child device failed.")
		return;
	}
...
void 
ImageTrans_Ctroller_servant::getConfigPropsFromPRF()
{
	FileSystem_impl * fs_i = new FileSystem_impl( m_fsroot.c_str() );
	if(!fs_i->exists(m_spdRelPath.c_str())) {
		DEBUG(0, [ImageTrans_Ctroller_servant::getConfigPropsFromPRF], 
			"SPD file is not existing: " << m_spdRelPath);
	}
 ...
 */
void 
ImageTrans_Ctroller_servant::query(
	CF::Properties & configProperties)
throw (
	CORBA::SystemException, 
	CF::UnknownProperties)
{
	DEBUG(6, [ImageTrans_Ctroller_servant::query], "In query.")
...
/**************************OPENSCA-USERREGION-BEGIN*******************************/
	...
	DEBUG(6, [ImageTrans_Ctroller_servant::query], " compLen: " << compLen);
	...
	if (0 == configProperties.length()){
		for(CORBA::Short i = 0; i < compLen; i++) {
			...
			DEBUG(6, [ImageTrans_Ctroller_servant::query], " len: " << len);
			CORBA::Short totalLen = totalProperties.length();
			DEBUG(6, [ImageTrans_Ctroller_servant::query], 
				" totalLen: " << totalLen);
 ...
 */
void 
ImageTrans_Ctroller_servant::configure(
...
{
	DEBUG(6, [ImageTrans_Ctroller_servant::configure], "In configure.")
	...
	try {
		...
		DEBUG(0, [ImageTrans_Ctroller_servant::configure], 
 ...
 */
void 
ImageTrans_Ctroller_servant::runTest(
	CORBA::ULong TestID, 
...
throw (
	...
	CORBA::SystemException)
{
	DEBUG(6, [ImageTrans_Ctroller_servant::runTest], "In runTest.")
/**************************OPENSCA-USERREGION-BEGIN*******************************/

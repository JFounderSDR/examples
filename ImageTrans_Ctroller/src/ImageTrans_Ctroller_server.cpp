/***************************************************************************//**
* @file     ImageTrans_Ctroller_server.cpp
* @author   open Team
* @version  1
* @date     2019-05-05
* @brief
...
void usage()
{
	DEBUG(0, [ImageTrans_Ctroller_Main], 
		" Example: ImageTrans_Ctroller_Main -DEBUG <deubg level>.")
	DEBUG(0, [ImageTrans_Ctroller_Main], 
		" Example: ImageTrans_Ctroller_Main -DEBUG 1.")
	DEBUG(0, [ImageTrans_Ctroller_Main], 
		" Example: ImageTrans_Ctroller_Main -ID DCE:fewf-wfe-34ss-fsef:1.")
	DEBUG(0, [ImageTrans_Ctroller_Main], 
		" Example: ImageTrans_Ctroller_Main -SFTFL device.spd.xml.")
	DEBUG(0, [ImageTrans_Ctroller_Main], 
		" Example: ImageTrans_Ctroller_Main --help.")
}
...
extern "C"
int ImageTrans_Ctroller_Main(int argc, char * argv[])
#elif defined __SDS_OS_LINUX__
int main(int argc, char * argv[])
#endif
{
	DEBUG(7, [ImageTrans_Ctroller_Main], "entering ImageTrans_Ctroller_Main...")
	...
	for (unsigned int numArg=0; numArg<argc; numArg++) {
		...
		if (strcmp(argv[numArg], "-ID") == 0) {
			...
			id = argv[numArg+1];
			DEBUG(7, [ImageTrans_Ctroller_Main], "id is " << id)
		}
		...
		if (strcmp(argv[numArg], "-COS_NAMING") == 0) {
			...
			DEBUG(7, [ImageTrans_Ctroller_Main], " cosNaming is " << cosNaming)
		}
		...
		if (strcmp(argv[numArg], "-APP_NAME") == 0) {
			...
			DEBUG(7, [ImageTrans_Ctroller_Main], " appName is " << appName)
		}
		...
		if (strcmp(argv[numArg], "-SFTFL") == 0) {
			...
			sftfl = argv[numArg+1];
			DEBUG(7, [ImageTrans_Ctroller_Main], "sftfl is " << sftfl)
		}
	}		///for (unsigned int numArg=0; numArg<argc; numArg++)
...
#ifdef __SDS_OS_VXWORKS__
	ImageTrans_Ctroller_servant * compServant = new ImageTrans_Ctroller_servant(
											id.c_str(), 
											cosNaming.c_str(), 
											appName.c_str(),
											sftfl.c_str(), 
	...
	DEBUG(7, [ImageTrans_Ctroller_Main], " begin to bind object with " << cosName)
	CF::Resource_var gppComp_v= compServant->_this();
	if (orbWrap->bind_object_to_string(
			gppComp_v.in(), cosName.c_str()) == false) {
		DEBUG(0, [ImageTrans_Ctroller_Main], "bind object with string failed.")
		delete orbWrap;
		delete compServant;
		return -1;
	}
	...
	DEBUG(7, [ImageTrans_Ctroller_server], " is running...")
	
#ifdef __SDS_OS_LINUX__
	try {
		DEBUG(7, [ImageTrans_Ctroller_server], " orb is running...")
		// run orb
		orbWrap->orb->run();
		// shutdown orb
		DEBUG(7, [ImageTrans_Ctroller_server], " orb is shutdowm...")
	} catch (...) {
		DEBUG(0, [ImageTrans_Ctroller_server], " run with unkown exception.")
	}
#endif
	...
	DEBUG(0, [ImageTrans_Ctroller_Main], " leaving ...")

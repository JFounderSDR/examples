...
void usage()
{
	DEBUG(0, [Zynq7035_PL_Main],
		" Example: Zynq7035_PL_Main -DEBUG <deubg level>.")
	DEBUG(0, [Zynq7035_PL_Main],
		" Example: Zynq7035_PL_Main -DEBUG 1.")
	DEBUG(0, [Zynq7035_PL_Main],
		" Example: Zynq7035_PL_Main -ID DCE:fewf-wfe-34ss-fsef:1.")
	DEBUG(0, [Zynq7035_PL_Main],
		" Example: Zynq7035_PL_Main -SFTFL device.spd.xml.")
	DEBUG(0, [Zynq7035_PL_Main],
		" Example: Zynq7035_PL_Main --help.")
}
...
extern "C"
int Zynq7035_PL_Main(int argc, char * argv[])
#elif defined __SDS_OS_LINUX__
int main(int argc, char * argv[])
#endif
{
	DEBUG(7, [Zynq7035_PL_Main], " entering Zynq7035_PL_Main...")
	...
	for (unsigned int numArg=0; numArg<argc; numArg++) {
		...
		if (strcmp(argv[numArg],"-DEBUG") == 0) {
			...
			if (isdigit(*argv[numArg+1])) {
				...
				DEBUG(0, [Zynq7035_PL_Main], " ERROR: illegal debug level given.")
				usage();
				exit(EXIT_FAILURE);
			}
		}
		...
		if (strcmp(argv[numArg], "-ID") == 0) {
			...
			id = argv[numArg+1];
			DEBUG(7, [Zynq7035_PL_Main], "id is " << id)
		}
		...
		if (strcmp(argv[numArg], "-LABEL") == 0) {
			...
			label = argv[numArg+1];
			devName = label;
			DEBUG(7, [Zynq7035_PL_Main], " label is " << label)
		}
		...
		if (strcmp(argv[numArg], "-SFTFL") == 0) {
			...
			sftfl = argv[numArg+1];
			DEBUG(7, [Zynq7035_PL_Main], "sftfl is " << sftfl)
		}
		...
		if (strcmp(argv[numArg], "-DEVMGRNAME") == 0) {
			...
			DEBUG(7, [Zynq7035_PL_Main],
				"devicemanager name is " << devmgrName)
		}

		///parser composite device ior
		if (strcmp(argv[numArg], "-COMPOSITE_DEVICE_IOR") == 0)
		{
			if (checkInParam(argc, numArg) == false)
				exit(EXIT_FAILURE);
			compositeDevIor = argv[numArg+1];
			DEBUG(7, [Zynq7035_PL_Main],
				"compositeDevIor is " << compositeDevIor)
		}
	}		///for (unsigned int numArg=0; numArg<argc; numArg++)

	/// instantiate Excutable_Device servant
#ifdef __SDS_OS_VXWORKS__
	Zynq7035_PL_servant * devServant = new Zynq7035_PL_servant(
											id.c_str(), 
											label.c_str(), 
											sftfl.c_str(),
											nodeName.c_str(), 
	...
	assert(devServant);
#ifdef __SDS_OS_VXWORKS__
	CF::LoadableDevice_var devServant_var = devServant->_this();
#elif defined __SDS_OS_LINUX__
	CF::LoadableDevice_var devServant_var =
		devServant->POA_CF::LoadableDevice::_this();
#endif
	...
	if (CORBA::is_nil(devServant_var)) {
		DEBUG(0, [Zynq7035_PL_Main], "bind device " << devName
	...
	///added to composite device
	try {
		CORBA::Object_var obj =
		orbWrap->ior_to_object( compositeDevIor.c_str() );
		CF::Device_var compositeDev = CF::Device::_nil();
		compositeDev = CF::Device::_narrow(obj.in());
		compositeDev->compositeDevice()->addDevice(devServant_var.in());
	} catch(...) {
		DEBUG(0, [Zynq7035_PL_Main], " get composite device failed.")
	}

	/// create device naming context by calling bind_new_context_with_string
	std::string compNaming = 
		"OpenSCA_Domain/" + nodeName + "/" + devName;
	if (orbWrap->bind_new_context_with_string( compNaming.c_str()) == false)
		throw CF::LifeCycle::InitializeError();
	
	DEBUG(7, Zynq7035_PL_server, " is running...")
#ifdef __SDS_OS_LINUX__
	try {
		DEBUG(7, Zynq7035_PL_server, " orb is running...")
		// run orb
		orbWrap->orb->run();
		// shutdown orb
		DEBUG(7, Zynq7035_PL_server, " orb is shutdowm...")
	} catch (...) {
		DEBUG(0, Zynq7035_PL_server, " run with unkown exception.")
	}
#endif
	...
	DEBUG(0, [Zynq7035_PL_server], " leaving ...")

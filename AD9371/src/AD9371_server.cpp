...
{
	DEBUG(0, [AD9371_Main],
		" Example: AD9371_Main -DEBUG <deubg level>.")
	DEBUG(0, [AD9371_Main],
		" Example: AD9371_Main -DEBUG 1.")
	DEBUG(0, [AD9371_Main],
		" Example: AD9371_Main -ID DCE:fewf-wfe-34ss-fsef:1.")
	DEBUG(0, [AD9371_Main],
		" Example: AD9371_Main -SFTFL device.spd.xml.")
	DEBUG(0, [AD9371_Main],
		" Example: AD9371_Main --help.")
}
...
{
	if (argvIndex + 1 == argc) {
		DEBUG(0, [AD9371_Main], " ERROR: illegal input parameter.")
		usage();
		return false;
	}
	return true;
...
extern "C"
int AD9371_Main(int argc, char * argv[])
#elif defined __SDS_OS_LINUX__
int main(int argc, char * argv[])
#endif
{
	DEBUG(7, [AD9371_Main], "entering AD9371_Main...")
	...
	for (unsigned int numArg=0; numArg<argc; numArg++) {
		...
		if (strcmp(argv[numArg],"-DEBUG") == 0) {
			...
			if (isdigit(*argv[numArg+1])) {
				...
				DEBUG(0, [AD9371_Main], " ERROR:illegal debug level given.")
				usage();
				exit(EXIT_FAILURE);
			}
		}
		...
		if (strcmp(argv[numArg], "-LABEL") == 0) {
			...
			label = argv[numArg+1];
			devName = label;
			DEBUG(7, [AD9371_Main], " label is " << label)
		}
		...
		if (strcmp(argv[numArg], "-SFTFL") == 0) {
			...
			sftfl = argv[numArg+1];
			DEBUG(7, [AD9371_Main], "sftfl is " << sftfl)
		}
		...
		if (strcmp(argv[numArg], "-NODENAME") == 0) {
			...
			DEBUG(7, [AD9371_Main], "node name is " << nodeName)
		}
		...
		if (strcmp(argv[numArg], "-DEVMGRNAME") == 0) {
			...
			DEBUG(7, [AD9371_Main], "devicemanager name is " << devmgrName)
		}
	}		///for (unsigned int numArg=0; numArg<argc; numArg++)

	/// instantiate Excutable_Device servant
#ifdef __SDS_OS_VXWORKS__
	AD9371_servant * devServant = new AD9371_servant(
											id.c_str(), 
											label.c_str(), 
											sftfl.c_str(),
											nodeName.c_str(), 
	...
	assert(devServant);
#ifdef __SDS_OS_VXWORKS__
	CF::Device_var devServant_var = devServant->_this();
#elif defined __SDS_OS_LINUX__
	CF::Device_var devServant_var = devServant->POA_CF::Device::_this();
#endif
	...
	if (CORBA::is_nil(devServant_var)) {
		DEBUG(0, [AD9371_Main], "bind device " << devName <<
	...
	DEBUG(7, [AD9371_server], " is running...")
	
#ifdef __SDS_OS_LINUX__
	try{
		DEBUG(7, [AD9371_server], " orb is running...")
		// run orb
		orbWrap->orb->run();
		// shutdown orb
		DEBUG(7, [AD9371_server], " orb is shutdowm...")
	}catch (...){
		DEBUG(0, [AD9371_server], " run with unkown exception.")
	}
#endif

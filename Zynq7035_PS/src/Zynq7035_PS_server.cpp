...
void usage()
{
	DEBUG(0, [Zynq7035_PS_Main],
		" Example: Zynq7035_PS_Main -DEBUG <deubg level>.")
	DEBUG(0, [Zynq7035_PS_Main],
		" Example: Zynq7035_PS_Main -DEBUG 1.")
	DEBUG(0, [Zynq7035_PS_Main],
		" Example: Zynq7035_PS_Main -ID DCE:fewf-wfe-34ss-fsef:1.")
	DEBUG(0, [Zynq7035_PS_Main],
		" Example: Zynq7035_PS_Main -SFTFL device.spd.xml.")
	DEBUG(0, [Zynq7035_PS_Main],
		" Example: Zynq7035_PS_Main --help.")
}
...
extern "C"
int Zynq7035_PS_Main(int argc, char * argv[])
#elif defined __SDS_OS_LINUX__
int main(int argc, char * argv[])
#endif
{
	DEBUG(7, [Zynq7035_PS_Main], " entering Zynq7035_PS_Main...")
	...
	for (unsigned int numArg=0; numArg<argc; numArg++) {
		...
		if (strcmp(argv[numArg], "-ID") == 0) {
			...
			id = argv[numArg+1];
			DEBUG(7, [Zynq7035_PS_Main], "id is " << id)
		}
		...
		if (strcmp(argv[numArg], "-LABEL") == 0) {
			...
			label = argv[numArg+1];
			devName = label;
			DEBUG(7, [Zynq7035_PS_Main], " label is " << label)
		}
		...
		if (strcmp(argv[numArg], "-SFTFL") == 0) {
			...
			sftfl = argv[numArg+1];
			DEBUG(7, [Zynq7035_PS_Main], "sftfl is " << sftfl)
		}
...
#ifdef __SDS_OS_VXWORKS__
	Zynq7035_PS_servant* devServant = new Zynq7035_PS_servant(
											id.c_str(),
											label.c_str(),
											sftfl.c_str(),
											nodeName.c_str(),
	...
	if (CORBA::is_nil(devServant_var)) {
		DEBUG(0, [Zynq7035_PS_Main], " bind device " << devName
	...
	try {
		...
		DEBUG(0, [Zynq7035_PS_Main], " get composite device failed.")
	}
	...
	DEBUG(7, [Zynq7035_PS_server], " is running...")
	
#ifdef __SDS_OS_LINUX__
	try {
		DEBUG(7, [Zynq7035_PS_server], " orb is running...")
		// run orb
		orbWrap->orb->run();
		// shutdown orb
		DEBUG(7, [Zynq7035_PS_server], " orb is shutdowm...")
	} catch (...) {
		DEBUG(0, [Zynq7035_PS_server], " run with unkown exception.")
	}
#endif
	...
	DEBUG(0, [Zynq7035_PS_Main], " leaving...")

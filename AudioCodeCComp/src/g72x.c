...
static short power2[15] = { 1, 2, 4, 8, 0x10, 0x20, 0x40, 0x80, 0x100, 0x200,
...
int size) /* table size of short integers */
{
	...
	if (d < 0) /* take 1 complement of i */
		return ((size << 1) + 1 - i);
	else if (i == 0) /* take 1 complement of 0 */
...
struct g72x_state *state_ptr) /* coder state pointer */
{
	 ...
	 */
	if (tr == 1) { /* reset a and b for modem signal */
		state_ptr->a[0] = 0;
		state_ptr->a[1] = 0;
		state_ptr->b[0] = 0;
		state_ptr->b[1] = 0;
...
int sign, short *qtab) {
	...
	if (id == i) { /* no adjustment on sp */
		...
		im = i ^ sign; /* 2 complement to biased unsigned */
		imx = id ^ sign;

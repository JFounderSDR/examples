...
void usage()
{
	DEBUG(0, [CRCComp_Main], 
		" Example: CRCComp_Main -DEBUG <deubg level>.")
	DEBUG(0, [CRCComp_Main], 
		" Example: CRCComp_Main -DEBUG 1.")
	DEBUG(0, [CRCComp_Main], 
		" Example: CRCComp_Main -ID DCE:fewf-wfe-34ss-fsef:1.")
	DEBUG(0, [CRCComp_Main], 
		" Example: CRCComp_Main -SFTFL device.spd.xml.")
	DEBUG(0, [CRCComp_Main], 
		" Example: CRCComp_Main --help.")
}
...
extern "C"
int CRCComp_Main(int argc, char * argv[])
#elif defined __SDS_OS_LINUX__
int main(int argc, char * argv[])
#endif
{
	DEBUG(7, [CRCComp_Main], "entering CRCComp_Main...")
	...
	for (unsigned int numArg=0; numArg<argc; numArg++) {
		...
		if (strcmp(argv[numArg], "-COS_NAMING") == 0) {
			...
			DEBUG(7, [CRCComp_Main], " cosNaming is " << cosNaming)
		}
		...
		if (strcmp(argv[numArg], "-APP_NAME") == 0) {
			...
			DEBUG(7, [CRCComp_Main], " appName is " << appName)
		}
		...
		if (strcmp(argv[numArg], "-SFTFL") == 0) {
			...
			sftfl = argv[numArg+1];
			DEBUG(7, [CRCComp_Main], "sftfl is " << sftfl)
		}
	}		///for (unsigned int numArg=0; numArg<argc; numArg++)
...
#ifdef __SDS_OS_VXWORKS__
	CRCComp_servant * compServant = new CRCComp_servant(
											id.c_str(), 
											cosNaming.c_str(), 
											appName.c_str(),
											sftfl.c_str(), 
	...
	DEBUG(7, [CRCComp_server], " is running...")
	
#ifdef __SDS_OS_LINUX__
	try {
		DEBUG(7, [CRCComp_server], " orb is running...")
		// run orb
		orbWrap->orb->run();
		// shutdown orb
		DEBUG(7, [CRCComp_server], " orb is shutdowm...")
	} catch (...) {
		DEBUG(0, [CRCComp_server], " run with unkown exception.")
	}
#endif
